...
const uint8_t M=1; // Number of MSG IDs with the max of 5.
const uint8_t N=6; // Number of normal ECUs with the max of 5. {1,2,3,4,5} are used in the paper. 
...
//  hash.update(Pre_shared_key_x, 16);
...
uint8_t recover_session_key(unsigned long canId, uint8_t auxY[N][16], uint8_t MAC[8], int m)
{
  uint8_t New_MAC[8];
  int tmp;
