...
const int N=2; // Number of normal ECUs with the max of 5. {1,2,3,4,5} are used in the paper. 

const int ArtDELAY1 = 80, ArtDELAY2 = 50; // Artifitial delay  


uint8_t Pre_shared_key_x[5][16]={
...
//  delay(ArtDELAY1);
  CAN.sendMsgBuf(ID, 1, 8, &R[n][0], true);
//  delay(ArtDELAY1);
  CAN.sendMsgBuf(ID, 1, 8, &R[n][8], true);
//  delay(ArtDELAY1);
  CAN.sendMsgBuf(ID, 1, 8, hmac, true);
//  delay(ArtDELAY1);
}

void send_kdmsg(uint8_t m)
{
  uint8_t hmac[8];
  uint8_t Rm[N][16];
  ...
  { 
    for(int i=0;i<N;i++)
    {
      ...
      points_y[i]=Pre_shared_key_y[i][b]^Rm[i][b];
    }
    points_x[N]=0;
...
//  delay(ArtDELAY1);
  for(int n=0;n<N;n++)
  {
    CAN.sendMsgBuf(ID, 1, 8, &auxY[n][0], true);
//    delay(ArtDELAY1);
    CAN.sendMsgBuf(ID, 1, 8, &auxY[n][8], true);
//    delay(ArtDELAY1);
  }
  //  delay(ArtDELAY1);
  CAN.sendMsgBuf(ID, 1, 8, hmac, true);
...
void setup()
{
  ...
  for(int m=0;m<M;m++)
  {
    ...
    if(m < M-1)
    {
      delay(ArtDELAY1); // Inter-KDMSG delay, giving ECU time to compute
    }
  }
  t5 = micros(); 
}
...
//      Serial.println();
      ...
      if(counterTT>=2*N)
      {
        ...
        Serial.print("Time for sending all KD_MSGs (ms): ");
        Serial.println((t5-t4)/1000);
        Serial.print("Time for entire session after key generation (ms): ");
        Serial.println((t6-t2)/1000);
        Serial.print("Time for entire session after preparation (ms): ");
        Serial.println((t6-t4)/1000);
        Serial.println();
