...
void send_prmsg(uint8_t Poly_para[M][N][16], uint8_t Pre_shared_key_x[N][16], uint8_t Pre_shared_key_y[N][16],uint8_t epoch[8])
{
  uint8_t Rtmp[16];
  uint8_t hmac[8];
...
void send_kdmsg(uint8_t Poly_para[M][N][16], uint8_t List[N-1], uint8_t epoch[8])
{
  unsigned long ID;
...
void setup()
{
  ...
  start2= micros(); 
  // Send out PR_MSGs
  send_prmsg(Poly_para, Pre_shared_key_x, Pre_shared_key_y, epoch);
  // Send out KD_MSGs
  start0 = micros();
  send_kdmsg(Poly_para, List, epoch);
